#!/usr/bin/env python3
"""校验「设定设计/」下 Markdown 内部相对链接、注册表路径与玩家 manifest。

用法（在仓库根目录）：
    python 设定设计/管线/检查/check-links.py
    python 设定设计/管线/检查/check-links.py --fix              # 尝试自动修正链接后复检
    python 设定设计/管线/检查/check-links.py --fix --stage-fixed  # 修正并把变更 git add（供 pre-commit）
...
class LinkIndex:
    ...
    def resolve(self, label: str, path_part: str) -> Path | None:
        """返回相对 设定设计/ 的目标路径；无法唯一确定时返回 None。"""
        label = label.strip()
...
def main() -> int:
    ...
    parser.add_argument(
        "--stage-fixed",
        action="store_true",
        help="与 --fix 联用：将设定设计/ 下已修改文件执行 git add（纳入本次提交，不 push）",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    if do_fix:
        ...
        if args.stage_fixed:
            ...
            if fix_log and not args.quiet:
                print("已将 设定设计/ 下变更加入暂存区（git add）。")
