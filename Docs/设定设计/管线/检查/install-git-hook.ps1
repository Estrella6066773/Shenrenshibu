...
try {
    ...
    if (-not $root) { throw "请在 Unity 工程根目录（含 Docs/）运行此脚本" }
    Set-Location $root

    git config core.hooksPath "设定设计/管线/检查"

    $legacy = Join-Path $root ".git\hooks\pre-commit"
    if (Test-Path $legacy) {
        @"
#!/bin/sh
# 已停用：提交前检查统一由 core.hooksPath=设定设计/管线/检查/pre-commit 负责。
exit 0
"@ | Set-Content -Path $legacy -Encoding utf8NoBOM
        Write-Host "已停用 .git/hooks/pre-commit（避免与 hooksPath 重复执行）"
    }

    Write-Host "已设置 core.hooksPath = 设定设计/管线/检查"
    Write-Host "提交时：仅当暂存区含 设定设计/*.md 时运行一次；成功时静默（--quiet）"
